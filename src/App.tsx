  ...
  const setHistoryList = setUsers;
    ...
    try {
      ...
      if (data && Array.isArray(data)) {
        // Mapeamento alinhado às colunas da planilha: row[0] = email, row[1] = status
    ...
    <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col items-center justify-center p-6 select-none overflow-hidden" id="admin-control-dashboard">
      ...
      >
        ...
        <div className="space-y-3">
          ...
          {/* Action buttons (Frosted glass styled) */}
          <div className="grid grid-cols-2 gap-3">
            {/* Green Check button (aprovar) */}
            <button
        ...
        {/* User License Management Panel / Real-time History Table */}
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center p-6 select-none overflow-hidden" id="rejected-access-gate">
      ...
      >
        ...
        {/* Support / Contact administrative button */}
...
function GoogleLoginPortal({ onLogin }: GoogleLoginPortalProps) {
  ...
  const handleVerifyEmail = (e: React.FormEvent) => {
    ...
    // Simulate gorgeous loading handshake steps
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col items-center justify-center p-6 select-none overflow-hidden" id="google-login-gate">
      ...
      >
        ...
        {/* Authentic-looking Google Sign-in Button */}
        {!isAuthenticating ? (
...
export default function App() {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed)) {
          // Purge corrupt "Moradia -R$ 5.647,00" or similar ghosts
          const cleanParsed = parsed.filter(t => {
            if (!t) return false;
            const desc = String(t.desc || '');
            const amountVal = Number(t.amount || (t as any).valor || 0);
            // Purge corrupt transactions with invalid formatting, negative amounts, or corrupt description strings
        ...
        // Clear corrupt storage safely to avoid crash
  ...
  // Current view date: default to June 2026 (the current active month in mock data)
  ...
  const handleGoogleLogin = async (email: string) => {
    ...
    if (formattedEmail === 'layonkell43@gmail.com') {
      // Admin auto-approval - bypass registration and instantly open secret Admin Control Dashboard
    ...
    // Modify the form submission when a user inputs their Gmail.
    // Right before displaying the locked "Aguardando Aprovação" glassmorphic screen,
    // trigger a background browser fetch request to send the email data straight to the Google script:
    try {
    ...
    // Standard first-time user flow: display secure Aguardando Aprovação gate
  ...
  // Load data from Google Script (GET) on login / startup
  ...
  // Save data to Google Script (POST) on manual actions
  ...
  // Automatically fetch from cloud when user is logged in AND approved
  useEffect(() => {
   if (userEmail && userEmail.trim() !== "" && userEmail !== "undefined" && licenseStatus === 'approved') {
  ...
  // --- LIMPAR CACHE TRAVADO DE LOGIN (CORREÇÃO DO LOOP ETERNO) ---
  useEffect(() => {
  ...
  // --- AUTOMATED BACKGROUND POLLING FOR LICENSE APPROVAL ---
  useEffect(() => {
    const ADMIN_EMAIL = 'layonkell43@gmail.com';
  ...
  // Check WebAuthn on component mount
  useEffect(() => {
  ...
  // Trigger automatic biometric verification on lock screen activation (bypassed for ADMIN_EMAIL to prevent bypass)
  useEffect(() => {
    const ADMIN_EMAIL = 'layonkell43@gmail.com';
  ...
  const handleBiometricVerify = async () => {
    ...
    try {
      ...
      console.warn("Native WebAuthn failed or cancelled. Using seamless iframe fallback.", err);
    }

    // Interactive simulator fallback to keep AI Studio container robust and fully functional
    // If native biometric fails (e.g. due to iframe permissions/sandbox), we run a highly polished scan animation
    // and seamlessly unlock to provide the absolute best user experience in the iframe!
    setTimeout(() => {
      setIsLocked(false);
      setLockInput('');
      setIsBiometricScanning(false);
  ...
  const handleBiometricRegister = async () => {
    ...
    try {
      ...
      console.warn("Native WebAuthn creation failed/cancelled or unsupported in iframe. Enabling local preference.", err);
    }
  ...
  const handleSavePin = (e: FormEvent) => {
    ...
    // If biometrics is supported and not enabled yet, prompt the user
  ...
  // Listen to physical keyboard keydown events when lock screen is active
  useEffect(() => {
    if (!isLocked) return;
  ...
  // Adjust default date field when the viewed month changes
  useEffect(() => {
  ...
  const cleanBRLToNumber = useCallback((val: any): number => {
    ...
    // If it contains both '.' and ',', it's likely European/Brazilian format (e.g. 1.234,56)
    if (s.includes('.') && s.includes(',')) {
      s = s.replace(/\./g, '').replace(/,/g, '.');
    } 
    // If it has a comma but no dot, replace comma with dot (e.g. 1234,56 -> 1234.56)
    else if (s.includes(',')) {
      s = s.replace(/,/g, '.');
    }
    // If it has a dot, we need to be careful if it's thousands separator (e.g. 10.000 -> 10000 or 1.23456 -> 1.23)
    else if (s.includes('.')) {
      const parts = s.split('.');
      if (parts.length > 2) {
        // Multiple dots, must be thousand separators: 1.234.567
        s = s.replace(/\./g, '');
      } else if (parts[1].length === 3) {
        // Single dot followed by exactly 3 digits is likely a thousand separator: 10.000 -> 10000
        s = s.replace(/\./g, '');
      }
    }
  ...
  // Active transactions for the currently viewed month
  const currentMonthTransactions = useMemo(() => {
    return getActiveTransactions(viewDate, transactions);
  }, [viewDate, transactions, getActiveTransactions, refreshHistory]);
  ...
  const handleAddOrEditCategory = () => {
    ...
    if (editingCategoryName) {
      ...
      // Update historical transaction category names
    ...
    setNewCatName('');
    setNewCatColor('#' + Math.floor(Math.random()*16777215).toString(16)); // Random color
  };
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (existingTx && existingTx.isFixed) {
      ...
      saveToCloud('save', { transactions: novasTransacoes, goals: novasMetas });
  ...
  const applyRecurringEdit = (scope: 'single' | 'future') => {
    ...
    saveToCloud('save', { transactions: updatedList, goals: goals });
    setRefreshHistory(prev => prev + 1);
  ...
  const handleGoalSubmit = (e: FormEvent) => {
    ...
    // Bulletproof Brazilian currency parser
    ...
    if (editingGoalId) {
      ...
      saveToCloud('save', { transactions: transactions, goals: updatedMetas });

      setEditingGoalId(null);
      
      // Forçar o React a rodar novamente o loop e recalcular os textos e a barra roxa na mesma hora
      setRefreshHistory(prev => prev + 1);
    } else {
  ...
  const handleDeleteGoal = (id: any) => {
    ...
    saveToCloud('save', { transactions: transactions, goals: updatedGoals });
  ...
  // Este useEffect garante que as recomputações dos top cards incluam o trigger de refresh
  useEffect(() => {
    // Força recomputação e garante a atualização instantânea do saldo, receitas e despesas
  }, [transactions, refreshHistory]);
  ...
  }, [currentMonthTransactions, viewDate, categories, refreshHistory]);
  ...
  const categoryColors = useMemo(() => {
    const colors: Record<string, string> = {
      'Sobra Líquida': '#14b8a6', // Mint Teal
  ...
  const marketAnalysis = useMemo(() => {
    ...
    let savingPercent = 20; // fallback to 20%
    let cheapestDayName = "";
  ...
  const filteredAndSortedLogs = useMemo(() => {
    // Current month's incomes and expenses
    ...
    // All investments from any month
  ...
  // --- CHART CALCULATIONS & GRAPHICS (SVG) ---
  ...
  // Step 0: If licenseStatus === 'expired' -> Render the Expired License screen
  if (licenseStatus === 'expired') {
    return (
  ...
  // Step 1: If userEmail is null -> Render the Google Login Screen
  if (!userEmail) {
    return (
      <GoogleLoginPortal
  ...
  // Step 2: If licenseStatus === 'pending' -> Render the Client Waiting Screen (Aguardando Aprovação)
...
if (!savedPin && userEmail !== 'layonkell43@gmail.com') {
  ...
  // Step 3: If isLocked === true -> Render the 4-digit PIN / Biometrics Keypad screen
  if (isLocked) {
    ...
    return (
      <div className="relative min-h-screen p-6 select-none bg-slate-950 flex flex-col items-center justify-center overflow-hidden" id="secure-lock-screen">
        ...
        <div className="max-w-xs w-full flex flex-col items-center space-y-8 relative z-10 text-center">
          ...
          {/* Large Biometric Scanning Scanner (if enabled) */}
          {isBiometricEnabled && (
  ...
  // Step 4: If unlocked AND userEmail === 'layonkell43@gmail.com' -> Render the Master Admin Control Dashboard (with History and Action Buttons)
  ...
  return (
    <div className="relative min-h-screen p-6 selection:bg-emerald-500/30 selection:text-emerald-200">
      ...
      <main className="max-w-6xl mx-auto space-y-6 relative z-10" id="main-content">
        ...
        {/* PREMIUM RESPONSIVE MULTI-TAB NAVIGATION */}
      ...
      {/* OVERLAY MODAL: DECIDE HOW TO SAVE EDITS ON FIXED TRANSACTION */}
      ...
      {/* BIOMETRIC ACTIVATION PROMPT DIALOG */}
      {showBiometricPrompt && (
